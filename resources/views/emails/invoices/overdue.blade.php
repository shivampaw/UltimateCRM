<p>Hi {{ $client->name }},</p>
<p>The invoice #{{ $invoice->id }} is now <strong>overdue by {{ $overdueDays }} day(s).</strong></p>
<p>The total charge for this invoice is <strong>{{ formatInvoiceTotal($invoice->total) }}</strong>.</p>
<p>The invoice was due by <strong>{{ $invoice->due_date->toFormattedDateString() }}</strong>.</p>
<p>You can pay for this invoice by logging on at
    <strong>{!! signedLoginUrl($client->user->id, '/invoices/' . $invoice->id) !!}</strong> and viewing your invoices.
</p>
<p>If you have any questions or problems be sure to email me, you can do so by replying to this email.</p>
<p>Thanks!<br/>
    {{ DB::table('users')->find(1)->name }}
</p>
